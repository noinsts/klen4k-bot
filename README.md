...
### Аукціон (`cogs/auction.py`)
- **`.add_item (name)`** - додаєте лот до аукціону.
- **`.auction`** - відображає всі активні лоти.
- **`.my_auc`** - відображає ваші активні лоти (або обраного користувача).
- **`.close_auc`** - закриває лот по його ID.
